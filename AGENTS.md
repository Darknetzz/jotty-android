...
- **GitHub release APK:** The `release-apk` workflow builds **`assembleDebug`** and attaches **`jotty-android-{VERSION_NAME}-debug.apk`** (debug-signed, installable, no repo secrets). For a **release** minified APK signed with your own keystore, build locally with `keystore.properties` (see `keystore.properties.example`).
