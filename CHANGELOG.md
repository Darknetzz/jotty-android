...
- **Visual note editor toolbar** — Formatting actions use compact icon buttons (matching the markdown toolbar) in a scrollable row so controls no longer clip off-screen; added underline, strikethrough, code, quote, image, and table actions.
...
- **New checklist items** — Adding a second item no longer fails with “Couldn’t save” after the first item syncs a newly created checklist to the server (stale local checklist id in detail view).
