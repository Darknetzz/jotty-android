...
- **Visual editor newlines** — Pressing Enter after existing text no longer glues the next line onto the previous one when viewing or saving (WebView `<div>` line breaks are converted with a proper newline).
