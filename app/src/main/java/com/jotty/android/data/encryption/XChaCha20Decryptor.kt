...
object XChaCha20Decryptor {
    ...
    private val ARGON2_PRESETS =
        listOf(
            // App + libsodium INTERACTIVE (64 MiB)
            Argon2Preset(2, 65536),
            // same with parallelism 2 (some bindings)
            Argon2Preset(2, 65536, 2),
            Argon2Preset(3, 65536),
            // Some libsodium bindings use 32 MiB
            Argon2Preset(4, 32768),
            Argon2Preset(2, 32768),
            // Min ops (e.g. test/default in some libs)
            Argon2Preset(1, 65536),
            // 128 MiB (capped; 256 MiB can OOM on devices)
     ...
     */
    fun decryptWithReason(
        encryptedBodyJson: String,
        passphrase: String,
    ): DecryptResult {
        val buf = passphrase.toCharArray()
        return try {
            decryptWithReason(encryptedBodyJson, buf)
        } finally {
            buf.clearPassphrase()
        }
    }
    ...
    /**
     * Same as [decryptWithReason] but accepts a mutable passphrase buffer; it is **cleared**
     * (zero-filled) before this call returns.
     */
    fun decryptWithReason(
        encryptedBodyJson: String,
        ...
        return try {
            decryptWithReasonOrThrowChar(encryptedBodyJson, passphrase)
    ...
    private fun decryptWithReasonOrThrowChar(
        encryptedBodyJson: String,
        passphrase: CharArray,
    ): DecryptResult {
        val variants = buildPassphraseVariants(passphrase)
        ...
        try {
            ...
            val presetList =
                buildList {
                    payload.jsonArgon?.let { add(it) }
                    addAll(ARGON2_PRESETS)
                }.distinctBy { Triple(it.iterations, it.memoryKb, it.parallelism) }
            val preferLibsodiumOrder = !payload.usedHex
            val nonce24Candidates =
                when {
            ...
            val firstTrimmed = variants.firstOrNull()
            for (pass in variants) {
    ...
    private fun buildPassphraseVariants(pass: CharArray): List<CharArray> {
        val trimmed = pass.copyTrimmedOrNull() ?: return emptyList()
        val built = mutableListOf<CharArray>()
        built.add(trimmed)
        built.add(normalizeNfc(trimmed))
        built.add(normalizeNfc(pass))
        if (!trimmed.contentEquals(pass)) {
            built.add(pass.copyOf())
        }
        return dedupePassphraseChars(built)
    }

    private fun normalizeNfc(chars: CharArray): CharArray = Normalizer.normalize(chars.concatToString(), Normalizer.Form.NFC).toCharArray()

    private fun dedupePassphraseChars(arrays: List<CharArray>): List<CharArray> {
        val out = mutableListOf<CharArray>()
        for (a in arrays) {
            if (out.none { it contentEquals a }) {
                out.add(a)
            } else {
                a.clearPassphrase()
            }
    ...
    fun decrypt(
        encryptedBodyJson: String,
        passphrase: String,
    ): String? = decryptWithReason(encryptedBodyJson, passphrase).plaintext
    ...
    private data class ParsedPayload(
        ...
        val data: ByteArray,
        val usedHex: Boolean,
        /** Argon2 params from JSON when present (e.g. libsodium / web); tried before preset list. */
        val jsonArgon: Argon2Preset? = null,
    ) {
        override fun equals(other: Any?) =
            (other is ParsedPayload) &&
                salt.contentEquals(other.salt) && nonce.contentEquals(other.nonce) &&
                data.contentEquals(other.data) && usedHex == other.usedHex && jsonArgon == other.jsonArgon

        override fun hashCode(): Int {
            var h = salt.contentHashCode()
            h = 31 * h + nonce.contentHashCode()
            h = 31 * h + data.contentHashCode()
            h = 31 * h + usedHex.hashCode()
            h = 31 * h + (jsonArgon?.hashCode() ?: 0)
            return h
        }
    }
    ...
    private fun parseEncryptedBody(json: String): Pair<ParsedPayload?, String?> {
        return try {
            ...
            ParsedPayload(salt, nonce, data, usedHex, body.resolveArgonPreset()) to null
    ...
    private data class EncryptedBodyJson(
        ...
        /** Argon2 iterations (short key used by some exporters). */
        @SerializedName("t") val t: Int? = null,
        /** Argon2 memory in KiB. */
        @SerializedName("m") val m: Int? = null,
        /** Argon2 parallelism. */
        @SerializedName("p") val p: Int? = null,
        @SerializedName("iterations") val iterations: Int? = null,
        @SerializedName("memory") val memory: Int? = null,
        @SerializedName("parallelism") val parallelism: Int? = null,
    )

    private fun EncryptedBodyJson.resolveArgonPreset(): Argon2Preset? {
        val iters = t ?: iterations ?: return null
        val memKb = m ?: memory ?: return null
        val par = p ?: parallelism ?: 1
        if (iters <= 0 || memKb <= 0 || par <= 0) return null
        return Argon2Preset(iters, memKb, par)
    }
    ...
    /** String overload for Gson/tests; allocates a temporary buffer. */
    private fun deriveKey(
        passphrase: String,
        salt: ByteArray,
        iterations: Int = ARGON2_ITERATIONS,
        ...
        val buf = passphrase.toCharArray()
        return try {
            deriveKey(buf, salt, iterations, memoryKb, parallelism)
        } finally {
            buf.clearPassphrase()
        }
    }
     ...
     */
    private fun decryptXChaCha20Poly1305(
        key: ByteArray,
        nonce24: ByteArray,
    ...
    internal fun hChaCha20Block(
        ...
        return byteArrayOf(
            *intToLeBytes(state[0]),
            *intToLeBytes(state[1]),
            *intToLeBytes(state[2]),
            *intToLeBytes(state[3]),
            *intToLeBytes(state[12]),
            *intToLeBytes(state[13]),
            *intToLeBytes(state[14]),
            *intToLeBytes(state[15]),
        )
    }
